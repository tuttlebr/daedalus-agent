...
// BEGIN GENERATED BRANDING (scripts/generate-branding.js; do not edit)
// prettier-ignore
const BRANDING = {
  "assets": {
...
const STATIC_ASSETS = [
  '/',
  ...Object.values(BRANDING.assets),
  BRANDING.manifest,
  '/offline.html',
];
// Auto-generated by scripts/inject-precache.js; do not edit manually
const BUILD_PRECACHE = [
  '/_next/static/chunks/webpack-52695e836171a24f.js',
  '/_next/static/chunks/framework-b9fd9bcc3ecde907.js',
  '/_next/static/chunks/main-980eb828d793b131.js',
  '/_next/static/css/5902abdc8b5f8849.css',
  '/_next/static/chunks/pages/_app-f39f1f1f6f9bcb95.js',
  '/_next/static/chunks/2bd9703c-4f40d9d3fd70c12d.js',
  '/_next/static/chunks/8942-d46066fa273a4e89.js',
  '/_next/static/chunks/pages/index-7e19f3a40c01e86c.js',
  ...
  event.waitUntil(
    Promise.all([
      caches.open(CACHE_NAME).then((cache) => {
        // Revalidate the unversioned shell/offline page even when the HTTP cache
        // still contains a response from the previous deployment.
        return cache.addAll(
          ALL_PRECACHE.map((url) => new Request(url, { cache: 'reload' })),
        );
      }),
      cacheManager.init(),
    ]),
...
self.addEventListener('fetch', (event) => {
  ...
  // Old open pages and installed apps can still request unversioned branding.
  // Revalidate these URLs instead of serving the old year-long HTTP/SW cache.
  if (
    url.origin === self.location.origin &&
    (BRANDING.assets[url.pathname] || url.pathname === '/manifest.json') &&
    url.pathname + url.search !==
      (BRANDING.assets[url.pathname] || BRANDING.manifest)
  ) {
    event.respondWith(
      ...
      fetch(request)
        ...
        .catch(async () => {
          ...
          const cachedResponse =
            (await runtime.match(request)) || (await precache.match(request));
          if (cachedResponse) return cachedResponse;

          // If not in cache and it's a navigation, try App Shell then offline page
          if (request.mode === 'navigate') {
            const appShell = await precache.match('/');
            if (appShell) return appShell;

            const offlinePage = await precache.match('/offline.html');
  ...
  // Read and refresh the SAME bucket. Global caches.match() could repeatedly
  // return a stale install entry even after its runtime copy had been refreshed.
  const precached = isPrecachedUrl(request.url);
  const cachePromise = caches.open(precached ? CACHE_NAME : RUNTIME_CACHE);
  const refresh = async (cache) => {
    const response = await fetch(request, { cache: 'no-cache' });
    const contentLength = response.headers.get('content-length');
    const size = contentLength ? parseInt(contentLength, 10) : 0;
    if (response.status === 200 && size < 5 * 1024 * 1024) {
      if (precached) await cache.put(request, response.clone());
      else await cacheManager.addToCache(cache, request, response.clone());
    }
    return response;
  };
...
self.addEventListener('push', (event) => {
  ...
  const options = {
    body: data.body,
    icon: BRANDING.assets['/favicon.png'],
    badge: BRANDING.assets['/favicon.png'],
    vibrate: [100, 50, 100],
    data: {
