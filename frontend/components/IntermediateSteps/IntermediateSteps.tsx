...
export const IntermediateSteps: React.FC<IntermediateStepsProps> = ({ steps, className = '', conversationId }) => {
  ...
  const activeConversationId = conversationId || selectedConversation?.id;
  const isConversationStreaming = Boolean(
    activeConversationId && streamingByConversationId[activeConversationId]
  );
  const resolvedIsStreaming = messageIsStreaming && isConversationStreaming;
