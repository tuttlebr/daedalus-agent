...
export const StatusStrip = forwardRef<HTMLButtonElement, StatusStripProps>(function StatusStrip(
  {
    config,
    activeRun,
    lastRunAt,
  ...
  const live = !!activeRun && ['running', 'queued', 'waiting_approval'].includes(activeRun.status);
    ...
    <div className="sticky top-0 z-30 -mx-4 mb-4 border-b border-white/[0.04] bg-[rgba(10,11,12,0.78)] px-4 py-3 backdrop-blur-xl md:-mx-6 md:px-6">
      <div className="mx-auto flex max-w-[720px] items-center gap-3">
        ...
        <div className="flex items-center gap-1">
          ...
          <IconChrome onClick={onOpenWorkspace} label="Open workspace" ref={workspaceButtonRef}>
    ...
    <span className="relative inline-flex h-2.5 w-2.5 shrink-0 items-center justify-center" aria-hidden>
      ...
      <span
        className={classNames(
          ...
          enabled ? 'bg-nvidia-green shadow-[0_0_10px_rgba(118,185,0,0.6)]' : 'bg-neutral-500',
        )}
      />
    </span>
  );
...
>(function IconChrome({ children, onClick, label }, ref) {
  return (
    <button
      ...
      onClick={onClick}
      aria-label={label}
      className="grid h-8 w-8 place-items-center rounded-full text-dark-text-muted transition hover:bg-white/[0.06] hover:text-dark-text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-nvidia-green/40"
    >
      {children}
    </button>
  );
