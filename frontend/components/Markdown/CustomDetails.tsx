...
export const CustomDetails = ({ children, id, messageIndex, index}) => {
    ...
    const shouldOpen = () => {
        ...
        // do NOT expand for top level (parsedIndex === -1) - keep it collapsed by default
        else {
            isOpen =  (expandIntermediateSteps && isLastMessage) || (isLastMessage && isLastIntermediateMessage && messageIsStreaming)
        }
        return isOpen
    }
    ...
    return (
        <>
            ...
            <span className={`text-left font-medium focus:outline-none transition-colors duration-300 hover:text-nvidia-green text-nvidia-green`}>
                {isLastMessage && messageIsStreaming && parsedIndex === -1 && (
                    <div className="relative mt-1 mb-2">
                    <div className="h-1 bg-gray-200 rounded-full overflow-hidden">
                        <div className="h-full bg-nvidia-green animate-loadingBar"></div>
                    </div>
                    </div>
                )}
            </span>
