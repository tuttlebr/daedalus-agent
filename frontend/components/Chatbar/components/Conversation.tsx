  ...
  return (
    <div className="relative flex items-center">
      {isRenaming && selectedConversation?.id === conversation.id ? (
        ...
        <button
          className={`flex w-full cursor-pointer items-center gap-3 rounded-xl p-3 text-sm transition-all duration-200 group/conv ${
            selectedConversation?.id === conversation.id
              ? 'apple-glass-subtle border border-nvidia-green/20 shadow-[0_0_12px_rgba(118,185,0,0.12)]'
        ...
        >
            <IconMessage size={18} className={selectedConversation?.id === conversation.id ? 'text-nvidia-green/80' : 'text-white/50 group-hover/conv:text-white/70'} />
          <div
            className={`relative max-h-5 flex-1 overflow-hidden text-ellipsis whitespace-nowrap break-all text-left text-[12.5px] leading-4 ${
              selectedConversation?.id === conversation.id
                ? 'text-white font-medium pr-12'
                : 'text-white/70 group-hover/conv:text-white/90 pr-1'
            }`}
          >
            {conversation.name}
          </div>
