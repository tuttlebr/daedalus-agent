...
import { IconPlus, IconPaperclip, IconCamera, IconBrain } from '@tabler/icons-react';
...
export const QuickActionsPopup: React.FC<QuickActionsPopupProps> = ({
  ...
  // Focus trap: keep Tab navigation within popup when open
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // Auto-focus first action when popup opens
      requestAnimationFrame(() => {
        const first = popupRef.current?.querySelector<HTMLElement>('button[role="menuitem"]');
        first?.focus();
      });
  ...
  useEffect(() => {
    if (isOpen && buttonRef.current) {
      ...
      // Always position above the button to prevent downward jump
      setPopupPosition({
