...
const UPLOAD_ENV_KEYS = [
  ...
  'UPLOAD_TRANSCRIPT_MAX_MB',
  'NEXT_PUBLIC_UPLOAD_MAX_IMAGES_PER_BATCH',
  'UPLOAD_MAX_IMAGES_PER_BATCH',
...
describe('UPLOAD_LIMITS env overrides', () => {
  ...
  it('uses document ingest request limit as a server-side fallback', async () => {
    const { UPLOAD_LIMITS: envLimits } = await importUploadLimitsWithEnv({
      DOCUMENT_INGEST_REQUEST_LIMIT: '300',
    });

    expect(envLimits.MAX_DOCUMENTS_PER_BATCH).toBe(300);
  });
