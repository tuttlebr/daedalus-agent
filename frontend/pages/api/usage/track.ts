...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  console.log('aiq - /api/usage/track called');
  ...
  try {
    ...
    console.log('aiq - /api/usage/track received:', {
      username,
      usage,
    ...
    if (!username || typeof username !== 'string') {
      console.error('aiq - /api/usage/track: invalid username:', username);
      return res.status(400).json({ error: 'Username is required and must be a string' });
    }

    if (!usage || typeof usage !== 'object') {
      console.error('aiq - /api/usage/track: invalid usage object:', usage);
    ...
    console.log('aiq - /api/usage/track: parsed usage fields:', {
      prompt_tokens,
      completion_tokens,
      total_tokens,
      types: {
    ...
    if (
      ...
      console.error('aiq - /api/usage/track: non-numeric values detected');
    ...
    console.log('aiq - /api/usage/track: calling trackUserUsage for', username);

    // Track the usage
    await trackUserUsage(username, usage as UsageData);

    console.log('aiq - /api/usage/track: successfully tracked usage for', username);
    ...
    console.error('aiq - /api/usage/track error:', error);
