...
This is a custom NVIDIA NeMo Agent toolkit workflow that integrates Milvus vector database as a retriever for building intelligent agents with vector search capabilities.
...
- **Database Support**: Can switch between different Milvus databases using the `database_name` parameter
...
# Use with NeMo Agent toolkit workflow
builder = Builder()
...
The reranking process includes the following steps:
...
The configuration class includes the following options:
...
The main retriever class provides the following methods:
