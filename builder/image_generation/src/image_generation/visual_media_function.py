...
async def visual_media_function(config: VisualMediaFunctionConfig, builder: Builder):  # noqa: ARG001
    ...
    async def _edit(prompt: str, imageRef: str | dict | list[dict] | None) -> str:
        if not prompt or not prompt.strip():
    ...
    async def _analyze(
        ...
        response = await _get_vlm_client().post(
            config.comprehension_api_endpoint.rstrip("/") + "/chat/completions",
            json=payload,
        )
    ...
    async def visual_media(
        ...
        try:
            ...
            if op == "edit":
                return await _edit(prompt, imageRef)
            if op == "analyze":
                return await _analyze(
                    question or prompt,
                    imageRef if not isinstance(imageRef, list) else None,
                    image_url,
                    videoRef,
                    video_url,
                    max_tokens,
            ...
            logger.error("Media API returned %d", exc.response.status_code)
    ...
    try:
        yield FunctionInfo.from_fn(
            ...
            description=(
                ...
                "about imageRef, image_url, videoRef, or video_url. Image outputs "
