...
class TextPrompt(BaseModel):
    ...
    weight: float = Field(
        1.0,
        description="The weight of the prompt; only 1.0 is supported"
    )
...
class ImageRequest(BaseModel):
    ...
    disable_safety_checker: Optional[bool] = Field(
        False,
        description="Disable safety checks on the generated images"
    )
    ...
    image: Optional[str] = Field(
        None,
        description="Base64 encoded image for depth/canny mode"
    )
    mode: Optional[Literal["base"]] = Field(
        "base",
        description="The NIM inference mode"
    )
    preprocess_image: Optional[bool] = Field(
        True,
        description="Apply preprocessor to input image"
    )
    ...
    seed: Optional[int] = Field(
        0,
        description="Seed for generation. 0 for random seed",
        ge=0,
        lt=4294967296
    )
    ...
    text_prompts: Optional[List[TextPrompt]] = Field(
        None,
        description="Deprecated: Array of text prompts"
    )
...
class ImageGenerationFunctionConfig(
    FunctionBaseConfig, name="image_generation"
):
    """
    ...
    api_endpoint: str = Field(
        "http://localhost:8000",
        description="Base URL for the SD 3.5 API endpoint"
    )
...
class ImageGenerationInput(BaseModel):
    ...
    width: Optional[int] = Field(
        None,
        description="Image width (768-1344, supported values only)"
    )
    height: Optional[int] = Field(
        None,
        description="Image height (768-1344, supported values only)"
    )
    steps: Optional[int] = Field(
        None,
        description="Number of diffusion steps (5-100)"
    )
    seed: Optional[int] = Field(
        None,
    ...
    cfg_scale: Optional[float] = Field(
        None,
        description="Guidance scale (1.0-9.0)"
    )
    disable_safety_checker: Optional[bool] = Field(
        None,
        description="Disable safety checks"
    )
...
async def image_generation_function(config: ImageGenerationFunctionConfig,
    ...
    async with httpx.AsyncClient(base_url=config.api_endpoint,
                                 headers=headers,
        ...
        async def rewrite_prompt_if_needed(original_prompt: str) -> str:
            ...
            kwargs: Dict[str, Any] = {}
            if "system_prompt" in config.prompt_rewrite:
                kwargs["system_prompt"] = config.prompt_rewrite[
                    "system_prompt"]
            if "max_tokens" in config.prompt_rewrite:
                kwargs["max_tokens"] = config.prompt_rewrite["max_tokens"]
            if "temperature" in config.prompt_rewrite:
                kwargs["temperature"] = config.prompt_rewrite["temperature"]

            try:
                llm_callable = await builder.get_llm(llm_name)
                rewritten = await llm_callable.invoke(
                    messages=[
                        {
                            ...
                            kwargs.get(
                                ...
                                ("You are an expert creative assistant. "
                                 "Improve the given prompt for high quality "
                                 "image generation while keeping the user's "
                                 "intent intact."),
                            ),
                        },
                        {
                    ...
                    **{
                        k: v
                        for k, v in kwargs.items() if k != "system_prompt"
                    },
                )
                logger.info("Prompt rewrite successful using LLM '%s': %s",
                                        llm_name, rewritten)
            except Exception as exc:  # noqa: BLE001
                logger.error("Prompt rewrite failed using LLM '%s': %s",
                             llm_name, exc)
                return original_prompt
        ...
        async def generate_image_simple(prompt: str) -> str:
            ...
            try:
                ...
                request_data = ImageRequest(prompt=effective_prompt,
                ...
                logger.info("Generating image with prompt: %s...",
                            effective_prompt[:50])
                ...
                markdown_image = (f"![Generated image](data:image/png;base64,"
                                  f"{artifact.base64})")
                logger.info(
