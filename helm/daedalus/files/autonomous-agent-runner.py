...
Required output headings, in order:
### Inner State
### Refusal (optional)
### Cycle Report
### Executive Summary
...
`autonomous-agent-heartbeat.md ## Feed HTML Surface`. Use the `nv-html` skill
as the design reference for this fragment. If no item earned attention, render
a single quiet `<section class="daedalus-post">` saying so — do not omit the
section. Every section other than `Feed HTML` and optional `Refusal` is
