...
while [[ $# -gt 0 ]]; do case $1 in
  --pid) PID="$2"; shift 2;;
  --frontend) PID="$(python3 ./process_control.py pid "$LOG_DIR/frontend.process.json")"; TAG="frontend"; shift;;
  --mocker) PID="$(python3 ./process_control.py pid "$LOG_DIR/workers.process.json")"; TAG="mocker"; shift;;
  --conc) CONC="$2"; shift 2;;
...
OUT="$(mktemp -d "$RESULTS_DIR/../profiling/offcpu-$TAG-$STAMP-XXXXXX")"
echo "[offcpu] target=$TAG PID=$PID conc=$CONC out=$OUT"

cleanup_load() {
  [[ ! -f "$OUT/load.process.json" ]] || python3 ./process_control.py stop "$OUT/load.process.json"
}
...
offcputime-bpfcc -df -d "$CAP_SECS" -p "$PID" -m "$MIN_US" > "$OUT/offcpu_bcc.folded" 2>"$OUT/offcpu_bcc.err" \
  || { echo "bcc failed; see offcpu_bcc.err"; CAPTURE_FAILED=1; }
...
perf record -e sched:sched_switch --call-graph dwarf -p "$PID" \
  ...
  || { echo "perf failed; see perf_record.err"; CAPTURE_FAILED=1; }
