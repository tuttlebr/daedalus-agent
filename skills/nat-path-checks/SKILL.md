...
Fix NeMo Agent Toolkit path-check failures with the smallest wording, escaping,
or link change that preserves meaning. Success means each reported token is
classified, fixed, and rechecked without hiding real broken paths.

Stop and ask for the failing path-check output when no concrete report or file
