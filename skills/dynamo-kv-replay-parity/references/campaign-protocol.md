# Frozen replay campaign protocol

Read this full protocol before starting a campaign. These are default gates for
this specific offline replay study, not instructions for unrelated benchmarks.
Record any user-requested protocol change before collecting samples and label
results against that declared scope.
...
[campaign concurrency](campaign-concurrency.md). Keep each row's
...
[internal-polynomial golden points](internal-polynomial-golden-points.md)
...
4. Retain canonical digests and the source reports needed to reproduce the decision.
...
Retain the campaign manifest, immutable inputs or their retrievable locations, reports,
checksums, all timing attempts, invalidations, schedules and profiler evidence needed to
reproduce the result. Remove only task-created disposable scratch outputs after their
evidence is safely retained. Honor user-supplied retention rules; never delete original
traces, unrelated caches or worktrees.
