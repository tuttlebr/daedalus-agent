...
Configure NeMo Agent Toolkit MCP clients, MCP servers, or serving entry points
using documented transport and server patterns. Success means the target
protocol, config surface, local smoke test, and failure behavior are explicit.

Stop and ask for the target transport, server mode, or credential boundary when
it cannot be inferred safely.
