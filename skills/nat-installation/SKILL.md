...
Set up or verify NeMo Agent Toolkit with the smallest package extras and first
runnable workflow that satisfy the request. Success means the install command,
environment assumptions, and verification command are explicit.

Stop and ask for the target environment or package manager when it cannot be
inferred from the repo or user request.
